...
const AddProject = (props) => {
    ...
    const handleClick = (event) => {
        ...
        if (project.title.length === 0) {
            alert("Title should have at least 1 character");
        }
        else if (project.description.length === 0) {
            alert("Description should have at least 1 character");
        }
        else {
            addProject(project.title, project.description, project.category, project.members_no, project.weeks, project.open_closed, project.status, project.skills_required, project.repository_link)
    ...
    return (
        <div className='container'>
            ...
            <form>
                ...
                <button type="submit" className="btn btn-success my-2" onClick={handleClick}>CreateProject</button>
            </form>
        </div>
    )
}
